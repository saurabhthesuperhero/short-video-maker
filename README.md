...
- Branch: `stoic/local-images`
- Purpose: Stoic short-video workflow using bundled local images instead of only Pexels footage
- Bundled custom assets in this branch: `84` Stoic images in `static/stoic` and `2` Stoic audio files in `static/audio`
- Key custom behavior: scenes can use `useLocalImage: true`, and images are served from `/api/static/images/:filename`
- Best fit: portrait StoicGodfather-style shorts with local art already committed to the repo
